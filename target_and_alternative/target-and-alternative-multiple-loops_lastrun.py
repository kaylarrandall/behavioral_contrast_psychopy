...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\micha\\OneDrive - Georgia Southern University\\4_RESEARCH\\ABA\\behavioral_contrast_psychopy\\target_and_alternative\\target-and-alternative-multiple-loops_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Instructions_phase_1 = visual.TextStim(win=win, name='Instructions_phase_1',
        text='Clicking on the white square sometimes get you points. When points are earned, click on the small white square that appears and you will earn your points. There is no pattern that you must learn. Please continue to click on the computer screen to earn points for the duration of the experiment. If you have questions, raise your hand and a researcher will come in and help you.',
        font='Arial',
    ...
    for thisOuter_trial in outer_trials:
        ...
        for thisInner_trial in inner_trials:
            ...
            main_square_screen = data.Routine(
                ...
                components=[logging_mouse, target, alternative, mouse_score, score_main_screen],
            )
            ...
            while continueRoutine:
                ...
                # if alternative is starting this frame...
                if alternative.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                    # keep track of start time/frame for later
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'alternative.started')
                    # update status
                    alternative.status = STARTED
                    alternative.setAutoDraw(True)
                
                # if alternative is active this frame...
                if alternative.status == STARTED:
                    # update params
                    pass
                # *mouse_score* updates
                ...
                if mouse_score.status == STARTED:  # only update if started and not finished!
                    ...
                    if buttons != prevButtonState:  # button state changed?
                        ...
                        if sum(buttons) > 0:  # state changed to a new click
                            ...
                            gotValidClick = False
                            clickableList = environmenttools.getFromNames([target,alternative], namespace=locals())
                ...
                if logging_mouse.getPressed()[0] == 0:
                    mouseDown = False
                ...
                if button.status == STARTED:
                    ...
                    if button.isClicked:
                        ...
                        if not button.wasClicked:
                            ...
                            print(mouse_score.clicked_name)
                            print(reinforced_square)
                            if str(mouse_score.clicked_name[0]) == reinforced_square:
                                score += points
